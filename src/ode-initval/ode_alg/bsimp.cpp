...
#include <ode-initval/ode_alg/odeiv_util.h>//<odeiv_util.h>
...
typedef struct
{
  CMatrix *d;                /* workspace for extrapolation         */
  CMatrix *a_mat;            /* workspace for linear system matrix  */
  CPermutation *p_vec;       /* workspace for LU permutation        */
                  ...
                  const gsl_odeiv_system * sys)
{
	...
	CMatrix *const a_mat = state->a_mat;
	CPermutation *const p_vec = state->p_vec;
	...
	CVectorView y_temp_vec(y_temp, dim);// = gsl_vector_view_array (y_temp, dim);
	CVectorView delta_temp_vec(delta_temp, dim);// = gsl_vector_view_array (delta_temp, dim);
	CVectorView rhs_temp_vec(rhs_temp, dim);// = gsl_vector_view_array (rhs_temp, dim);
	...
	for (i = 0; i < dim; i++)
    {
		for (j = 0; j < dim; j++)
        {
			(*a_mat)[i][j] = -h* dfdy[i][j];//gsl_matrix_set (a_mat, i, j, -h * gsl_matrix_get (dfdy, i, j));
        }
		(*a_mat)[i][i] = dfdy[i][i] + 1.0;//gsl_matrix_set (a_mat, i, i, gsl_matrix_get (a_mat, i, i) + 1.0);
    }
	
	/* LU decomposition for the linear system. */
	
	(*a_mat).LU_decomp(p_vec, &signum);
	...
	//delta_temp_vec = y_temp_vec;
	(*a_mat).LU_solve(*p_vec, y_temp_vec, delta_temp_vec);
	...
	for (n_inter = 1; n_inter < n_step; n_inter++)
    {
		...
		//delta_temp_vec = rhs_temp_vec;
		(*a_mat).LU_solve(*p_vec, rhs_temp_vec, delta_temp_vec);
...
bsimp_alloc (size_t dim)
{
  ...
  state->d = new CMatrix(SEQUENCE_MAX, dim);//gsl_matrix_alloc (SEQUENCE_MAX, dim);
  state->a_mat = new CMatrix(dim, dim);//gsl_matrix_alloc (dim, dim);
  state->p_vec = new CPermutation(dim);// gsl_permutation_alloc (dim);

  state->yp = (double *) new double[dim];//malloc (dim * sizeof (double));
  state->y_save = (double *) new double[dim];//malloc (dim * sizeof (double));
  state->yerr_save = (double *) new double[dim];//malloc (dim * sizeof (double));
  state->y_extrap_save = (double *) new double[dim];//malloc (dim * sizeof (double));
  state->y_extrap_sequence = (double *) new double[dim];//malloc (dim * sizeof (double));
  state->extrap_work = (double *) new double[dim];//malloc (dim * sizeof (double));
  state->dfdt = (double *) new double[dim];//malloc (dim * sizeof (double));
  state->y_temp = (double *) new double[dim];//malloc (dim * sizeof (double));
  state->delta_temp = (double *) new double[dim];//malloc (dim * sizeof(double));
  state->weight = (double *)new double[dim];// malloc (dim * sizeof(double));

  state->dfdy = new CMatrix(dim, dim);//gsl_matrix_alloc (dim, dim);

  state->rhs_temp = (double *) new double[dim];//malloc (dim * sizeof(double));
  state->delta = (double *) new double[dim];//malloc (dim * sizeof (double));

  {
	  size_t k_choice = bsimp_deuf_kchoice (CMath::SQRT_DBL_EPSILON, dim); /*FIXME: choice of epsilon? */
    state->k_choice = k_choice;
    state->k_current = k_choice;
    state->order = 2 * k_choice;
  }

  state->h_next = -CMath::SQRT_DBL_MAX;//GSL_SQRT_DBL_MAX;
             ...
             const gsl_odeiv_system * sys)
{
  ...
  {
    int s = GSL_ODEIV_JA_EVAL (sys, t_local, y, dfdy->GetDataPtr(), dfdt);
...
bsimp_free (void * vstate)
{
  ...
  delete [] state->delta;//free (state->delta);
  delete [] state->rhs_temp;//free (state->rhs_temp);

  delete state->dfdy;//gsl_matrix_free (state->dfdy);

  delete [] state->weight;//free (state->weight);
  delete [] state->delta_temp;//free (state->delta_temp);
  delete [] state->y_temp;//free (state->y_temp);
  delete [] state->dfdt;//free (state->dfdt);
  delete [] state->extrap_work;//free (state->extrap_work);
  delete [] state->y_extrap_sequence;//free (state->y_extrap_sequence);
  delete [] state->y_extrap_save;//free (state->y_extrap_save);
  delete [] state->y_save;//free (state->y_save);
  delete [] state->yerr_save;//free (state->yerr_save);
  delete [] state->yp;//free (state->yp);

  delete state->p_vec;//gsl_permutation_free (state->p_vec);
  delete state->a_mat;//gsl_matrix_free (state->a_mat);
  delete state->d;//gsl_matrix_free (state->d);
...
const gsl_odeiv_step_type *gsl_odeiv_step_bsimp = &bsimp_cpp::bsimp_type;
