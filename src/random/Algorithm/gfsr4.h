...
#define C 6988
#define D 9689
#define GFSR4_M 16383 /* = 2^14-1 */
/* #define M 0x0003fff */

typedef struct
  {
    int nd;
    unsigned long ra[GFSR4_M+1];
  }
gfsr4_state_t;
...
gfsr4_get (void *vstate)
{
  ...
  state->nd = ((state->nd)+1)&GFSR4_M;
  return state->ra[(state->nd)] =
      state->ra[((state->nd)+(GFSR4_M+1-A))&GFSR4_M]^
      state->ra[((state->nd)+(GFSR4_M+1-B))&GFSR4_M]^
      state->ra[((state->nd)+(GFSR4_M+1-C))&GFSR4_M]^
      state->ra[((state->nd)+(GFSR4_M+1-D))&GFSR4_M];
...
const gsl_rng_type *gsl_rng_gfsr4 = &gfsr4::gfsr4_type;
