...
#define KNUTHRAN_MM (1L << 30)           /* the modulus */
#define TT 70                   /* guaranteed separation between streams */

#define evenize(x) ((x) & (KNUTHRAN_MM - 2))     /* make x even */
#define is_odd(x) ((x) & 1)     /* the units bit of x */
#define mod_diff(x, y) (((x) - (y)) & (KNUTHRAN_MM - 1)) /* (x - y) mod MM */
  ...
  for (j = 0; j < KK; j++)
    {
      ...
      ss <<= 1;
      if (ss >= KNUTHRAN_MM)             /* cyclic shift 29 bits */
        ss -= KNUTHRAN_MM - 2;
    }
  for (; j < KK + KK - 1; j++)
    x[j] = 0;
...
const gsl_rng_type *gsl_rng_knuthran = &knuthran::ran_type;
