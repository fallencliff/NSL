...
#ifndef NSL_GSL_ERRNO_H__
#define NSL_GSL_ERRNO_H__
...
#define GSL_STATUS_UPDATE(sp, s) do { if ((s) != GSL_SUCCESS) *(sp) = (s);} while(0)
	...
	NSL_EXPORT void gsl_error (const char * reason, const char * file, int line,
		int gsl_errno);
	
	NSL_EXPORT void gsl_stream_printf (const char *label, const char *file,
		int line, const char *reason);
	
	NSL_EXPORT const char * gsl_strerror (const int gsl_errno);
	...
	NSL_EXPORT FILE * gsl_set_stream (FILE * new_stream);
...
#define GSL_ERROR_NULL(reason, gsl_errno) GSL_ERROR_VAL(reason, gsl_errno, 0)
	...
	*/
	...
	{ 
			...
			GSL_ERVIEW   = 33		/*resize a view of matrix or vector*/
	}NSL_ERR_CODE;

#endif // NSL_GSL_ERRNO_H__
